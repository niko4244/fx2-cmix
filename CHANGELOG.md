...
- **Adam t<LIMIT w-update rebuilt to the baseline's exact FP chain**
  (`lstm-layer.hpp` `Adam(...)`): the reciprocal-of-den1 refinement is
  `t1 = rr*rcp; t2 = den1*t1 - rr (vfmsub231); q = t1 - t2*rcp
  (vfnmadd213)` — the rounded product is `den1*t1`, NOT `rr*den1 - t1`
  (which rounds a different product) and NOT the fast-math re-associated
  `rr - den1*t1`. The chain is wrapped in
  `reassociate(off) contract(off)` with `t1`/`t2` routed through
  volatile round-trips, and the loop is split so the 8-wide vectorized
  blocks use that rcp-refine chain while the scalar remainder uses a
  plain `rr/den1` (vdivss) — exactly how the baseline compiles the
  ...
  All three were the byte-158 divergence that failed the output-identity
  ...
  `LstmLayer::BackwardPass(NeuronLayer&)` and the out-of-line
  `(anonymous namespace)::Adam` symbol in the baseline. Mirrored in

...
inline void LstmLayer::ForwardPass(NeuronLayer& neurons,
  ...
  // Scalar loops replace valarray temporaries; reduction and element ops are
  // in the same order as the valarray versions ((norm*norm).sum() accumulates
  // sequentially, matching the loop below).
  float sum = 0;
  for (unsigned int i = 0; i < num_cells_; ++i) {
    float n = neurons.norm_[epoch_][i];
    sum += n * n;
  }
...
inline void LstmLayer::BackwardPass(NeuronLayer& neurons,
  ...
  // Scalar loops replace valarray temporaries; element ops and reduction
  // order are identical to the valarray expressions.
  for (unsigned int i = 0; i < num_cells_; ++i) {
  ...
  const float mean = sum / num_cells_;
  for (unsigned int i = 0; i < num_cells_; ++i) {
  ...
  for (unsigned int i = 0; i < num_cells_; ++i) {
    ...
    for (unsigned int j = 0; j < input_size; ++j) {
      neurons.update_[i][output_size_ + j] += e * input[j];
    }
